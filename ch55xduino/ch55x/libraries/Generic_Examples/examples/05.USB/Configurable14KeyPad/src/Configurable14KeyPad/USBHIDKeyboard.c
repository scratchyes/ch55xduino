...
__xdata uint16_t HIDConsumer = 0x0000;
...
uint8_t USB_EP3_send(__data uint8_t reportID) {
  if (UsbConfig == 0) {
    return 0;
  }
  ...
  __data uint8_t sendLen = 0;
  if (reportID == HID_REPORT_ID_KEYBOARD) {
    Ep3Buffer[0] = HID_REPORT_ID_KEYBOARD;
    for (__data uint8_t i = 0; i < sizeof(HIDKey); i++) {
      Ep3Buffer[1 + i] = HIDKey[i];
    }
    sendLen = 1 + sizeof(HIDKey);
  } else if (reportID == HID_REPORT_ID_CONSUMER) {
    Ep3Buffer[0] = HID_REPORT_ID_CONSUMER;
    Ep3Buffer[1] = (uint8_t)(HIDConsumer & 0xFF);
    Ep3Buffer[2] = (uint8_t)(HIDConsumer >> 8);
    sendLen = 3;
  } else {
    return 0;
  }
  ...
  UEP3_T_LEN = sendLen;         // data length
  UpPoint3_Busy = 1;
...
uint8_t Keyboard_press(__data uint8_t k) {
  ...
  USB_EP3_send(HID_REPORT_ID_KEYBOARD);
  return 1;
}
...
uint8_t Consumer_press(__data uint16_t k) {
  HIDConsumer = k;
  return USB_EP3_send(HID_REPORT_ID_CONSUMER);
}
...
void Consumer_releaseAll(void) {
  HIDConsumer = 0;
  USB_EP3_send(HID_REPORT_ID_CONSUMER);
}
